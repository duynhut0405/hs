        ...
        await Promise.all(
          currentCart?.map(async (item) => {
            ...
            if (result.data.status === '200') {
              const find = result.data.result_data.map((item) => item.store_code === store_id);
              if (Number(item.qty) < Number(find?.total_quantity_primary)) {
                err.push(item.name);
              }
            }
          })
  ...
  const setPayNow = async () => {
    ...
    if (payMethod.code == 'checkmo') {
      try {
        ...
        alert('Thanh toán không thành công. Vui lòng thử lại sau!');
        return;
      }
    } else {
      try {
        const result = await api.post(`carts/mine/set-payment-information`, {
          billingAddress: {
            ...
            firstname: commonState.firstname + ' ' + commonState.lastname,
            lastname: ' ',
            postcode: '12345',
            saveInAddressBook: null,
            street: commonState.street,
                            ...
                            {store &&
                              store.map((item, index) => (
                                <option value={item.store_code} key={index}>
                                  {item.name?.split('-')[0] + ' - ' + item.address?.replace(new RegExp(process.env.REPLACE_LOCATION, 'g'), '') + (item?.phone ? ' - ' + item?.phone : '')}
                                </option>
                              ))}
                          </select>
                        </div>
