...
function AddressModalMobile({ showLocation }) {
  const [menuRef, setMenuRef] = useState(React.useRef(null));
  const { location, setActiveLocationModal, activeLocationModal } = useCommon();
    ...
    <>
      { location && (
          <div className="form-search-header">
            {showLocation && <span className="t1"> {t('choise_area')} </span>}
...
AddressModalMobile.propTypes = propTypes;
