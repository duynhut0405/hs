  ...
  return (
    ...
    <LayoutMobileOnlyHead/>
      {notConfirm && !checkoutSuccess &&
          ...
          <h1>Thanh toán <span className="small">({total} Sản phẩm)</span></h1>
        </div>
      }
      {!notConfirm && !checkoutSuccess &&
          <div className="page-top-heading" onClick={() => setNotConfirm(true)}>
          <a className="back icon-arrow-1 ix" href="#"></a>
          <h1>Xác nhận <span className="small">({total} Sản phẩm)</span></h1>
        </div>
      }
      {!checkoutSuccess && <><main className={`page-cart ${isDeleted ? 'loading' : ''}`}>
        {state ? (
          <>
              {currentCartInfo != null && currentCartInfo.extension_attributes && currentCartInfo.extension_attributes.shipping_assignments && currentCartInfo.extension_attributes.shipping_assignments[0] && (
                <ShippingDefault setCaculating={setCaculating} data={currentCartInfo.extension_attributes.shipping_assignments[0].shipping} notConfirm={notConfirm} category={state} setCheckoutSuccess={setCheckoutSuccess} isPayNow={isPayNow} setShippingAddress={setShippingAddress} setNotConfirm={setNotConfirm}/>
              )}
              {notConfirm &&
                <>
                <div className="pd-15 ">
      ...
      {!notConfirm && (caculatedResult && caculatedResult.coupon_code) && 
          ...
          <div className="row">
            <div className="col-6">
              <a className="btn btn-4 full" onClick={() => setIsPayNow(true)}>Mua Hàng</a>
            </div>
            <div className="col-6">
              <a className="btn full" href="./">Mua thêm</a> 
            </div>
          </div>
      </div>
        }
        ...
        {notConfirm && (caculatedResult && caculatedResult.coupon_code) &&
          <div className={`w-final-price ${isCaculating ? 'loading' : ''}`}>
              ...
              <div className="row">
                <div className="col-6">
                  <a className="btn btn-4 full" onClick={() => setNotConfirm(false)}>Xác nhận</a>
                </div>
                <div className="col-6">
                  <a className="btn full" href="./">Mua thêm</a> 
                </div>
      ...
      <HeaderMobile/>
        <main className="sec-tb page-success">
          ...
          <div className="entry-heading text-center">
              ...
              <div className="desc">Mã đơn hàng: <span className="w5">{checkoutSuccess.increment_id}</span>
              </div>
          </div>
          <div className="box-shadow box-product mb-20 box-confirm ">
            <div className="pd-20">
                  ...
                  <div>
                      <span className="w5">{checkoutSuccess.customer_firstname} {checkoutSuccess.customer_lastname}</span> <br/>
              ...
              <div className="widget-total-cart ">
                ...
                <div className="items">
                  <div className="r1">
                      <span className="pull-left cl4">Tạm tính</span>
                      <span id="total-price" className="b">{formatVND(checkoutSuccess.base_subtotal)}</span>  đ
                  </div>
                  <div className="r1">
                      <span className="pull-left cl4">Phí vận chuyển</span>
                      <span id="charges" className="b" price="150000">{formatVND(checkoutSuccess.payment.base_shipping_amount)}</span> đ 
                  </div>
                </div>
                {checkoutSuccess.coupon_code && checkoutSuccess.coupon_code.length > 0 && (
                  <div id="coupons" className="items coupons active">
                      <h6 className="title" >Mã khuyến mãi <span className="code extend">{checkoutSuccess.coupon_code}</span></h6>
                      <p className="r1 extend mb-10">
                          <span className="pull-left cl4">Giảm</span>
                          <span id="coupon" price="50000" className="b">{checkoutSuccess.discount_amount}</span>  đ
                      </p>     
                  </div> 
                )}
                <div className="items">
                      <div className="r1">
                        <span className="pull-left b">Tổng tiền</span>
                        <span  className="b cl1"><span id="final-price">{formatVND(checkoutSuccess.base_grand_total)}</span> đ</span>  
                    </div> 
                </div> 
              </div>
          </div>
