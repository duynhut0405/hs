...
function ShippingDefault({setCaculating, data, notConfirm, category, isPayNow, setCheckoutSuccess, setShippingAddress, setNotConfirm}) {
  const {createCartForGuest} = useAuth();
          ...
          const result = await api.post(`/guest-carts/${guestId}/estimate-shipping-methods`, {
            "address": {
              ...
              "custom_attributes": {
                ...
                "district": {"attribute_code": "district", "value": currentDistrict.full_name},
                "district_id": {"attribute_code": "district_id", "value": currentDistrict.value},
                "ward": {"attribute_code": "ward", "value": currentWard.full_name},
  ...
  const submit = async (data) => {
    ...
    if (guestId) {
      try {        
        ...
        const result1 = await api.post(`/guest-carts/${guestId}/shipping-information`, {
          "addressInformation": {
            "billing_address": {
              "city": currentCity.label,
              "company": null,
            ...
            "shipping_carrier_code": result.data[0].method_code,
            "shipping_method_code": result.data[0].carrier_code
          }
        })
    ...
    {notConfirm && !setted && location && (
      ...
      <div className="sec-bd-bottom  box-cart-5" style={{display:"block"}}>
        ...
        <div className="row list-item-20">
            <div className="col-md-6">
                <label className="full">
                  ...
                  <input  className="input style-2" placeholder="Tên" name="firstname" ref={register({ required: true })} value={currentState.firstname} onChange={(e) => setCurrentState({...currentState, firstname: e.target.value})}/>
            ...
            <div className="col-md-6">
                <label className="full">
                  ...
                  <input  className="input style-2" placeholder="Họ" name="lastname" ref={register({ required: true })}  value={currentState.lastname} onChange={(e) => setCurrentState({...currentState, lastname: e.target.value})}/>
            ...
            <div className="col-md-6">
                <label className="full">
                  ...
                  <input  className="input style-2" placeholder="Số điện thoại" name="telephone" ref={register({ required: true })}  value={currentState.telephone} onChange={(e) => setCurrentState({...currentState, telephone: e.target.value})}/>
            ...
            <div className="col-md-6">
                <label className="full">
                  ...
                  <input  className="input style-2" placeholder="Số nhà, tên đường" name="street" ref={register({ required: true })}  value={currentState.street[0]} onChange={(e) => setCurrentState({...currentState, street: [`${e.target.value}`]})}/>
        ...
        <div className="row list-item-20">
            ...
            <div className="col-md-4">
              <select id="address_QH" className="select select-quan"  value={currentDistrict.value} name="district" onChange={(e) => changeDistrict(e)} ref={register({ required: true })}>
