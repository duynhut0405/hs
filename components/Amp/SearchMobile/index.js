...
function SearchMobile({ menu, query, initFiltersNavigation, productsData, currentFilter }) {
  ...
  const { activeFilterModal, setActiveFilterModal, keyword, setKeyWord } = useCommon();
  ...
  //     let link = "/#";
  //     if (element.additional_data) link = element.additional_data.request_path;
  ...
  return (
    <>
      <LayoutMobile menu={menu} >
        ...
        <SectionBreadcrumbMobile data={[{ name: "Search", link: "/#", isActive: true }]} />

        <section className="" >
          <div className="row-border-bottom row-border-bottom-page-title ">

          {keyword ? (
            <h1 className="h2 page-title mb-10">Kết quả tìm kiếm cho "{keyword}" <br />
          ...
          (<h1 className="h2 page-title mb-10">Tìm sản phẩm <br /></h1>)
          } 
          </div>
          ...
          <div className="row-border-bottom">
            <div className="filter-category ">
                ...
                <select className="select" onChange={(e) => {
                  ...
                  {list_sort != undefined && (
                    Object.keys(list_sort).map((item, index) => (
                      <option value={item} key={index} name={'product_list_order'} > {list_sort[item]} </option>
                    ))
                  )}
                </select>
              </div>
              ...
              <span className="menu-btn x" onClick={() => (showMenu())}><span></span></span>

              <a className="filter-btn btnModal" data-modal="fiterModal" onClick={() => { setActiveFilterModal(true) }} > <i className="icon-t16"></i> Lọc</a>
            </div>
                ...
                <ul className="accordion-menu accordion">
                  {(menu != undefined) && (menu.map((item, index) => (
                    <li className="children" key={index}>
                      <Link href={item.additional_data != undefined ? '/' + item.additional_data.request_path : '/'}><a ><span> {item.name} </span></a></Link>
                      <ul>
                        {(item.children != undefined) && (item.children.map((child, child_index) => (
                          <li key={child_index}><Link href={child.additional_data != undefined ? '/' + child.additional_data.request_path : '/'}><a ><span> {child.name} </span></a></Link></li>
                        ))
                        )}
                      </ul>
                    </li>
          ...
          <div className="pd-15">
            <div className="list-filter">
              <span className="t1 reset" onClick={() => clearFilter()}> {t('clear_filter')} </span>
              {filters.cat && activeCategory && activeCategory.name && (
                <a className="t1 a"> {activeCategory.name} <i className="icon-close" onClick={() => { removeCategoryFilter(activeCategory.id) }}></i></a>
              )}

              {filters.rating != '' && rating != '' && (
                <a className="t1">{t('from')} {filters.rating} {t('stars')} <i className="icon-close" onClick={() => { removeRatingFilter() }}></i></a>
              )}
              ...
              {filters.custom_filters.length != 0 && (
                Object.keys(filters.custom_filters).map((item, index) => (
                  <a className="t1" key={index}>{filters.custom_filters[item].label} <i className="icon-close" onClick={() => { removeCustomFilter(item, index) }} ></i></a>
                ))
              )}
            </div>
            ...
            <div className="div-bd-bottom">
              <div className="row list-item list-p-1">
                {(clientSideProductsData.items != undefined) && (clientSideProductsData.items.map((item, index) => (
                  <div className="col-lg-c5 col-md-3 col-sm-4 col-6 " key={index}>
                    <CategoryProductMobile data={item} />
                  </div>
                ))
                )}
          ...
          <>
            <div id="fiterModal" className={`myModal fiterModal ${activeFilterModal ? 'active' : ''}`}>
              <div className="container ">
                <div className="top">
                  <a className="back btnModal icon-arrow-1 ix" data-modal="fiterModal" onClick={() => { setActiveFilterModal(false) }}></a> <span className="w6">Lọc sản phẩm</span>
                  <a className="btnModal btn-close" data-modal="fiterModal" onClick={() => { setActiveFilterModal(false) }}><i className="icon-close"> </i></a>
                </div>
                <div className="contentModal">
                  <div className="widget widget-menu">
                    <h6 className="widget-title"> {t('categories')} </h6>
                    <ul className="menu row">
                      {(initFiltersNavigation && initFiltersNavigation.cat && initFiltersNavigation.cat.value != undefined) && (initFiltersNavigation.cat.value.map((item, index) => (
                        <li className="col-4" key={index} value={item.value} onClick={(e) => toggleCategory(item)}>
                          <a className={activeCategory.id == item.value ? "active" : ""}> {item.label} </a></li>
                      ))
                      )}
                    </ul>
                  </div>
                    ...
                    <div className="widget widget-menu">
                      ...
                      <ul className="menu row">
                        {initFiltersNavigation.brand.value.map((item, index) => (
                          <li className="col-4" key={index} > <a className={brand.items.includes(item) ? "active" : ""}
                            onClick={() => {
                    ...
                    <ul className="menu row">
                      {list_rating != undefined && (
                        Object.keys(list_rating).map((item, index) => (
                          <li className="col-4" key={index} onClick={() => toggleRating(item)}><a className={rating == item ? "active" : ""} ><i className="icon-star rated"></i> <span> {list_rating[item]} </span></a></li>
                        ))
                      )}
                    </ul>
                  </div>
                      ...
                      (item != 'brand' && item != 'cat' && item != 'price' && item != 'rating') && (
                        <div className="widget widget-menu" key={index}>
                          ...
                          {initFiltersNavigation[item].value.map((val, i) => (
                            <li key={i}>
                              <a className={customFT.filter[item] && (customFT.filter[item] == val) ? "active" : ""}
                                onClick={() => {
