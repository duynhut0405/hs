...
function SectionPopularCatalogMobile({ data, menu }) {
  ...
  data.details.forEach(item => {
    ...
    popular_cat.push({
      request_path: category_obj.additional_data.request_path,
      url: item.url,
      alt: item.alt
    })
  });
        ...
        <div className="row list-item list-b-2  lazy-hidden group-ef">
          {(popular_cat != undefined) && (popular_cat.map((item, index) => (
            <div className={`col-6 efch-${index} ef-img-t`} key={index} >
              <Link href={item ? "/" + item.request_path.replace(".html", "") : '/'}>
                <a className="item " >
                  <div className="img tRes "><img className="lazy-hidden" data-lazy-type="image" data-lazy-src={`${process.env.DOMAIN_IMAGE + item.url}`} src="/images/no-image.svg" alt={item.alt} /></div>
