...
function QuestionMobile({ commonData, productId }) {
    ...
    <>
      <div className='pd-15 sec-bd-bottom'>
        ...
        {/* <p className="text-center">
          <a href="#" className="btn">Xem thêm câu hỏi</a>
        </p> */}
      </div>
      <div className='pd-15 sec-bd-bottom'>
        <h3>Đặt câu hỏi</h3>
        <form className={`${loading ? 'loading' : ''}`} onSubmit={handleSubmit(submit)}>
          <textarea maxLength='1000' className='input' name='question' ref={register({ required: true })} value={question} onChange={(e) => setQuestion(e.target.value)}></textarea>
          <br />
          <br />
          <p>
            <button type={submit} className='btn btn-4 pull-right'>
              Gửi câu hỏi
            </button>
