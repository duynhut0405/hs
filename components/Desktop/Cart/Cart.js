...
function Cart({}) {
  const { cart, isAuthenticated, getTotalGuestInfo, caculatedResultAll, setGetCartAgain } = useAuth();
  ...
  const findPromotion = async () => {
    ...
    if (!isAuthenticated && guestId) {
      let price = 0;
      try {
        const result = await api.get(`service/guest-carts/${guestId}/promotion-gift`);
      } catch (error) {
        throw error;
      }
    }
  ...
  const getCartLater = async () => {
    const id = Cookies.get('cardId');
    if (!!id) {
      try {
  ...
  const setCheckAll = async (e, type) => {
  ...
  const uncheck_item_cart1 = async (list, userID) => {
    const guestId = Cookies.get('cardId');
    const urlDel = `guest-carts/${guestId}/items/`;
    try {
      const arr = [];
  ...
  const check_item_cart2 = async (list) => {
    const guestId = Cookies.get('cardId');
    const urlAddCart1 = `guest-carts/${guestId}/items/`;
      ...
      <main className={`sec-tb page-cart`}>
        <div className='container'>
          ...
          {state ? (
                    ...
                    <table className='table table-line productListCart'>
                        ...
                        <tr>
                          <th>
                            <label className='checkbox  '>
                              <input name='radio' type='checkbox' checked={isChecked} onChange={(e) => setCheckAll(e, 'cart-1')} />
                              <span></span>
                      ...
                      <tbody>
                        {Object.keys(state).map((item, index) => {
                          ...
                          return (
                            <Category
                              getCartLater={() => getCartLater(Cookies.get('cardId'))}
                              data={state[item]}
                              cate={item}
                              key={index}
                              ...
                              uncheck_item_cart1={(data) => {
                                setCaculating(true);
                                uncheck_item_cart1(data, Cookies.get('cardId'));
                              }}
                              setState={setState}
                              state={state}
                            />
                      ...
                      <div className='items'>
                        {caculatedResult.total_segments.map((item, index) => {
                          switch (item.code) {
                            case 'grand_total':
                              return (
                                <div className='r1 mb-20' key={index}>
            ...
            <span className='red2'>Chưa có sản phẩm nào cả</span>
          )}
        </div>
        {state2 && (
          <div className='container'>
                    ...
                    <table className='table table-line productListCart'>
                      <thead>
                        <tr>
                          <th>
                            <label className='checkbox  '>
                              <input name='radio' type='checkbox' checked={isChecked2} onChange={(e) => setCheckAll(e, 'cart-2')} />
                              <span></span>
                              <strong>Sản phẩm</strong>
                            </label>
                          </th>
                        ...
                        {Object.keys(state2).map((item, index) => {
                          ...
                          return (
                            <Category
                              ...
                              check_item_cart2={(data) => check_item_cart2(data, Cookies.get('cardId'))}
                              cart={cart}
                            />
                          );
                        })}
