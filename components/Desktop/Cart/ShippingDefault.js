...
import LoginModalCheckOut from '../Common/LoginModalCheckOut'
...
function ShippingDefault({ data, notConfirm, category, isPayNow, setCheckoutSuccess, setShippingAddress, setNotConfirm }) {
  const { createCartForGuest, getLoginOtp, login, setOpenModalCheckout, signUp } = useAuth();
  ...
  const { location, setLocationData } = useCommon();
      ...
      if (payMethod.code == 'checkmo') {
        try {
          ...
          if (result.data.extension_attributes.masked_id) {
            Cookie.set('cardId', result.data.extension_attributes.masked_id);
            return;
          }
          createCartForGuest();
        } catch (error) {
        ...
        try {
          ...
          if (result.data) {
            const result = await api.get(`guest-carts/${guestId}/vnpay?return=${encodeURI(window.location.href + '/success')}`)
            // window.open(
            //   result.data
            // );
    ...
    <>
      {notConfirm && setted && location && (
        <div className="box-shadow mb-20 box-cart-4">
          ...
          <div className="pd-20">
            {currentState.lastname}   {currentState.firstname} {currentState.telephone} <br />
            {currentState.street}, {currentWard.label}, {currentDistrict.label}, {currentCity.label}
          </div>
          <div className="action">
            <a className="" href="#" >Mặc định</a> |  <a className="" onClick={() => { changeAlreadySetBefored(false) }} >Đổi địa chỉ</a>
          </div>
        </div>
      )}
      {notConfirm && !setted && location && (
            ...
            <form onSubmit={handleSubmit(submit)}>
              <div className="pd-20">
                <div className="row list-item-20">
                  ...
                  <div className="col-md-6">
                    <label className="full">
                      ...
                      <input className="input style-2" placeholder="Số nhà, tên đường" name="street" 
                        ref={register({
                          required: {
                            value: true,
                ...
                <div className="row list-item-20">
                  ...
                  <div className="col-md-4">
                    <select id="address_QH" className="select select-quan" data-parent="address_Tinh" data-child1="address_PX" defaultValue={currentDistrict.value} name="district" onChange={(e) => changeDistrict(e)} ref={register({ required: true })}>
      ...
      {notConfirm && setted && shippingMethod && (<div className="box-shadow mb-20 box-cart-3">
        ...
        <div className="box-shadow box-product mb-20 box-confirm ">
          <div className="pd-20">
            ...
            <div>
              ...
              {currentState.street}, {(currentState.custom_attributes.find(element => element.attribute_code == 'ward')).value}, {(currentState.custom_attributes.find(element => element.attribute_code == 'district')).value}, {currentState.city}
            </div>
          </div>
          <div className="pd-20">
