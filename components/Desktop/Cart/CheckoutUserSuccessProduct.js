...
function CheckoutUserSuccessProduct({ item }) {
  ...
  return (
    value && (
      <div className='item'>
        <Link href={`/product/${getLastSlashProduct(value.extension_attributes.url_key)}`}>
          <a href={`/product/${getLastSlashProduct(value.extension_attributes.url_key)}`} className='img tRes_68'>
        ...
        <div className='divtext'>
          <div className='line-3 title'>
            <a href={`/product/${getLastSlashProduct(value.extension_attributes.url_key)}`}>{value.name}</a>
          </div>
          <div className='price'>
            <strong>{formatVND(value.price)} đ</strong> <br />
          </div>
          {value.extension_attributes.options.map((data, index) => (
            <div className='action' key={index}>
              <span className='number'>
                {data.label}: <span style={{ color: 'red' }}>{ReactHtmlParser(data.value)}</span>
              </span>
              {data.label == 'Chiều dài' && totalLength && (
                <>
                  <br />
          ...
          {value?.extension_attributes?.extra_info && (
            <div>
              <span style={{ fontSize: '14px', display: 'inline-block' }}>{'Loại sóng '}: &nbsp; </span>
              <span style={{ fontSize: '14px', display: 'inline-block' }}>
                <span style={{ color: 'red' }}>{value?.extension_attributes?.extra_info}</span>
              </span>
            </div>
          )}
          <div className='action'>
            <span className='number'>
              {t('number')}: {value.qty_ordered}
            </span>
          </div>
        </div>
      </div>
...
export default CheckoutUserSuccessProduct;
