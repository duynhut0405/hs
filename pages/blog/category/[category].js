...
import BlogBox from '../../../components/Desktop/Common/BlogBox'
...
function Blog({ data, sidebar, page, isMobile }) {
  ...
  const handlePageChange = (pageNumber) => {
    router.push(`/blog/category/${router.query.category}?page=${pageNumber}`)
  }
  const setShowMenu = () => {
  ...
  return (
    !isMobile ?
    <Layout>
      <BreadCrumb data={[{name: 'Tin tức', link: '/blog', isActive: false}, {name: data.title, link: `/blog/category/${data.identifier}`, isActive: true}]}/>
      <section className=" sec-tb  sec-bd-bottom" >
        <div className="container">
          <div className="row ">
            <div className=" col-lg-9">
              <div className=" box-shadow">
                <h1 className="box-title-3">{data.title}</h1>
                <div className="pd-20">
              ...
              <div className="sidebar-inner">
                <div className="widget box-shadow">
                  ...
                  <ul className="accordion accordion-menu">
                    ...
                    {sidebar.categories && sidebar.categories.map((item, index) => (
                      <li className={`children ${item.data.identifier == data.identifier ? 'active' : ''}`} key={index}>
                ...
                <div className="widget box-shadow">
                  ...
                  <div className=" list-item list-b-8">
                    {sidebar.popular &&  sidebar.popular.map((item, index) => (
                  ...
                  <div className="pd-20">
                    <Link href="/blog">
                      <a href="/blog" className="viewall">Xem tất cả <i className="icon-arrow-2"></i></a>
                    </Link>
                  </div>
                </div>
              </div>
    ...
    <LayoutMobile>
      <HeaderMobile/>
      <SectionBreadcrumbMobile data={[{name: 'Tin tức', link: '/blog', isActive: false}, {name: data.title, link: `/blog/category/${data.identifier}`, isActive: true}]}/>
      <div className="blog-heading">
        <h1 className="h2">{data.title}</h1>
        <span className="menu-btn x" onClick={() => setShowMenu()}><span></span></span>
      </div>
      <div id="wmenu-blog">
      ...
      <section className=" sec-post-feature  group-ef lazy-hidden">
        ...
        <div className="pd-15 text-center">
          <Link href="/blog">
            <a href="/blog" className="viewall cl1">Xem tất cả <i className="icon-arrow-2"></i></a>
          </Link>
        </div>
      </section>
      <br></br>
...
export async function getServerSideProps(context) {
  ...
  try {
    const result = await Promise.all([
      api.get(`service/rewrite/entity/blog*category*${encodeURI(category)}?p=${page}`),
